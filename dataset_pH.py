...
class EnzymeData:
    ...
    def from_header(cls, header, sequence):
        id_, organism, ec, ph, *other = map(str.strip, header.split('|'))
        return cls(id=id_, ph_label=float(ph), sequence=sequence, ec_label=ec)
...
class DatasetPhClassification(DatasetBase):
    ...
    def __getitem__(self, idx):
        return self.data.loc[idx, ['accession', 'sequence', 'ph']]
